...
def test_1iter(pytestconfig):
    ...
    verilog_sources = [ 
    PROJECT_PATH / "divider_unsigned.v",
    ...
    runr.test(
        ...
        testcase=None, # filter tests via the `--tests` command-line flag
    )
    pass
...
@cocotb.test()
async def test_corner_max_values(dut):
    """Corner Case: Dividing the largest possible 32-bit integer"""
    await Timer(1, "ns")
    # 0xFFFFFFFF is the max 32-bit value (4,294,967,295)
    dut.i_dividend.value = 0xFFFFFFFF 
    dut.i_divisor.value = 1
    await Timer(1, "ns")
    # Result should be the number itself
    assertEquals(0xFFFFFFFF, dut.o_quotient.value, "Max Value / 1 failed")
    assertEquals(0, dut.o_remainder.value, "Max Value / 1 remainder failed")

    await Timer(1, "ns")
    dut.i_dividend.value = 0xFFFFFFFF
    dut.i_divisor.value = 0xFFFFFFFF
    await Timer(1, "ns")
    # Result should be 1, remainder 0
    assertEquals(1, dut.o_quotient.value, "Max / Max failed")
    assertEquals(0, dut.o_remainder.value, "Max / Max remainder failed")

@cocotb.test()
async def test_corner_zero_dividend(dut):
    """Corner Case: 0 divided by anything should be 0"""
    await Timer(1, "ns")
    dut.i_dividend.value = 0
    dut.i_divisor.value = 12345
    await Timer(1, "ns")
    assertEquals(0, dut.o_quotient.value)
    assertEquals(0, dut.o_remainder.value)

@cocotb.test()
async def test_corner_small_div_big(dut):
    """Corner Case: Dividend is smaller than Divisor"""
    await Timer(1, "ns")
    dut.i_dividend.value = 50
    dut.i_divisor.value = 100
    await Timer(1, "ns")
    # Quotient should be 0, Remainder should be the Dividend (50)
    assertEquals(0, dut.o_quotient.value)
    assertEquals(50, dut.o_remainder.value)

@cocotb.test()
async def test_alternating_bits(dut):
    """Edge Case: Alternating bit patterns (0xAAAAAAAA / 0x55555555)"""
    # 1010... divided by 0101... -> roughly 2
    await Timer(1, "ns")
    dividend = 0xAAAAAAAA
    divisor = 0x55555555
    dut.i_dividend.value = dividend
